...
```{r}
dm = matrix(NA, nrow = 100, ncol = 3)
dm[ ,1] = as.numeric(x_3=="infraction")
dm[ ,2] = as.numeric(x_3=="felony")
dm[ ,3] = as.numeric(x_3=="misdimeanor")
colnames(dm) = c("infraction", "felony", "misdimeanor")
dm
```

* What should the sum of each row be (in English)? 

The sum should be either 0 or 1 because there is only one place to put an object.
...
The column sum should be the average of it.
...
```{r}
n = 100
dm = matrix(data = NA, nrow = 100, ncol = 6)
dm[, 1] = rnorm(n = 100, mean = 17, sd = sqrt(38))
dm[, 2] = runif(n = 100, min = -10, max = 10)
dm[, 3] = rpois(100, lambda = 6)
dm[, 4] = rexp(100, rate = 9)
dm[, 5] = rbinom(100, size = 20, prob = 0.12)
dm[, 6] = sample(c(rep(1, 100 * .24), rep(0, 100 * .76)))
...
```{r}
?data.frame
d_frame = data.frame(dm)
?factor
d_frame$ X6 = factor(d_frame$X6, levels = c(0,1), labels = c("DOMESTIC", "FOREIGN"))
?View
View(d_frame)
```
...
?print
?table
print(table(d_frame$X6))
print(table(d_frame$X6)/n)
```
...
?matrix
n=50
R = matrix(sample(c(rep(0,2500 * 0.5), rep(1,2500 * 0.25), replicate(2,2500 * 0.25))), nrow = 50, ncol = 50)
print(R)
```
...
```{r}
?apply
apply(X>0, MARGIN = 2, sum, na.rm = TRUE)
```
...
?min
?is.na
lapply(split(X, col(X)), function(X){list(min = min(X, na.rm=TRUE), max = max(X, na.rm = TRUE), pct_missing = (sum(is.na(X))/n), first_NA = min(which(is.na(X))))})
```
...
```{r}
?set.seed
X = set.seed(X, kind = NULL, normal.kind = NULL, sample.kind = NULL)
n = 1000
?Normal
...
?Normal
quantile(v, 0)
qnorm(0, mean = mean(v), sd = sd(v)) #Theoretically
```
