...
for(i in 1:8){
  my_list[[array[i]]] = array(1: i, dim = c(rep(i, times = i)))
  }
```
...
?object.size
#The output above shows an estimate of the memory being stored
#Yes, since later arrays that use more bytes
...
name_type$M$Boomer = strsplit("Theodore, Bernard, Gene, Herbert, Ray, Tom, Lee, Alfred, Leroy, Eddie", split = ", ")[[1]]
name_type$M$GenX = strsplit("Marc, Jaime, Greg, Darryl, Tim, Dean, Jon, Chris, Troy, Jeff", split = ", ")[[1]]
name_type$M$Millenial = strsplit("Zachary, Dylan, Christian, Wesley, Seth, Austin, Gabriel, Evan, Casey, Luis", split = ", ")[[1]]
...
name_type$F$Millenial = strsplit("Samantha, Alexis, Brittany, Lauren, Taylor, Bethany, Latoya, Candice, Brittney, Cheyenne", split = ", ")[[1]] 
name_type
```
...
```{r eval = FALSE}
v= seq(-100, 100)
expect_equal(v, -100 : 100) #changed the second variable in () from 101 to 100 b/c not working
```
...
#' @param y_binary    The vector of training data labels 
#' @param Xtest       A test observation as a row vector
#' @param d           The function of distance which uses two row vectors as its inputs 
#' @return            The predicted label for the test observation
...
#From the data you can tell the each one of its S.D is not far from each other.
...
#It does because we can see that as the threshold parameter increases the theshold model performance decreases.
...
#' @param w           The parameter that belongs to the final parameter weight
#'
...
TO-DO: 
#The graph represents the first_feature in the x-axis and the #second_feature in the y-axis. It sem]ems that the 0 response tended #to be closer to zero or the center where as the 1 responses were #placed farther away the center. 
...
#This numbers represent the weight which was mentioned to be the #computed final parameter.
#The intercept of the line would be w_0 and the slope would be the #w_1. 
...
#The line of seperation is not satisfying because it only really takes #into account the 0 response, leaving the 1 responses as outliers.
